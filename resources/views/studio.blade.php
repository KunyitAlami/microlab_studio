    ...
    const scenes = [
        ...
        function scene1() {
            ...
            const alat = [
                ...
                { nama: "Spiritus", fungsi: "Sebagai sumber api untuk sterilisasi ose bulat.", gambar: imgSpiritus },
                { nama: "Cawan Petri (NA Plate)", fungsi: "Berisi media nutrient agar tempat tumbuh bakteri.", gambar: imgCawan },
                { nama: "Tabung Bakteri", fungsi: "Berisi suspensi bakteri yang akan diinokulasi.", gambar: imgTabungTampilan },
                { nama: "Inkubator", fungsi: "Tempat inkubasi agar bakteri tumbuh optimal.", gambar: imgInkubator },
                { nama: "Rak Tabung", fungsi: "Tempat untuk meletakan tabung bakteri sebelum dan sesudah digunakan", gambar: imgRakTabung },
            ];
        ...
        function scene3() {
            ...
            setTimeout(() => {
                ...
                if (imgTabung && imgTabung.complete && imgTabung.naturalWidth){ 
                    ...
                    tabungWidth = 120; tabungHeight = 300; 
                }
        ...
        function scene4() {
            ...
            let isSteril = false; 
            let sceneActive = true;
            const button = { x: 20, y: 120, width: 120, height: 40 };
            let ose = { 
                x: canvas.width / 2 - 50, 
...
function scene5() {
    ...
    const button = { x: 20, y: 120, width: 120, height: 40 };
    const ambilButton = { x: canvas.width/2 - 100, y: canvas.height - 100, width: 200, height: 40 };
    const nextButton = { x: canvas.width - 150, y: canvas.height - 60, width: 120, height: 40 };
    ...
    setTimeout(() => {
        function getInstructions() {
            switch(currentStage) {
                case 1: return "Instruksi: Nyalakan spiritus terlebih dahulu.";
                case 2: return "Instruksi: Miringkan tabung bakteri sekitar 30° ke kanan (ke area api spiritus).";
                case 3: return "Instruksi: Drag ose bulat ke tabung bakteri untuk mengambil spesimen.";
                case 4: return "Instruksi: Tahap selesai! Lanjut ke tahap berikutnya.";
                default: return "Instruksi: Ikuti petunjuk di layar.";
            }
        }
        ...
        function activateDrawing() {
            ...
            function drawPattern(x, y) {
                if (isPointInArea(x, y, currentStep)) {
                    areas[currentStep].strokePoints.push({x, y});
...
function scene7() {
    ...
    setTimeout(() => {
        ...
        let cawaPos = { x: canvas.width/2, y: canvas.height/2+300 };
        let cawaSize = { width: 200, height: 80 };
        let inkubatorArea = { 
    ...
    function scene8(){
        ...
        setTimeout(() => {
            ...
            const texts = [
                {title: "Hasil", content: "Melalui inokulasi gesekan pada bakteri E. coli, kita dapat mengetahui lima langkah utama dalam teknik ini, yaitu: persiapan bakteri, pengambilan inokulum, penggoresan pada media, isolasi koloni, dan pengamatan pertumbuhan. Metode gesekan bertujuan untuk menyebarkan bakteri secara bertahap agar koloni tunggal dapat terbentuk, memungkinkan identifikasi morfologi koloni secara jelas."},
                {title: "Hasil pada bagian 50% pertama", content: "Pada area 50% pertama, koloni tumbuh rapat dan berdekatan karena menerima konsentrasi bakteri paling tinggi, tampak krem hingga putih kekuningan, bulat, dan mengkilap, menunjukkan inokulum berhasil diaplikasikan."},
                {title: "Hasil pada bagian 25% pertama (sekunder)", content: "Pada area 25% pertama, koloni mulai tersebar lebih merata dan mulai terpisah, menandakan teknik gesekan efektif untuk isolasi awal."},
                {title: "Hasil pada bagian 25% kedua (terakhir)", content: "Sedangkan pada area 25% kedua, koloni sangat terisolasi sehingga hampir semua koloni tunggal, ideal untuk pengambilan sampel murni."},
                {title: "Kesimpulan", content: "Dari keseluruhan proses ini, dapat disimpulkan bahwa inokulasi gesekan berhasil menyebarkan bakteri dari area padat ke terisolasi, memungkinkan identifikasi morfologi koloni dan isolasi koloni tunggal untuk penelitian lanjutan."}
            ];
